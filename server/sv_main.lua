...
AddEventHandler('qb-vault:server:checkThePassword', function(password,citizenid,storagename)
    local src = source
    local storage = exports.oxmysql:executeSync('SELECT storagename FROM vaults WHERE citizenid = ? And storagename = ? And password = ?',{citizenid,storagename,password})
...
AddEventHandler('qb-vault:server:checkStorageExist', function(citizenid,storagename,location)
    ...
    local storage = exports.oxmysql:executeSync('SELECT storagename FROM vaults WHERE citizenid = ? And storagename = ?',{citizenid,storagename})
